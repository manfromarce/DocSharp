...
internal class Builder
{
        ...
        while (groups.Count > 0)
        {
            while (groups.Peek().Tokens.MoveNext())
            {
                ...
                if (token is Group childGroup)
                {
                    ...
                    else if (dest is ListTextFallback || dest is NumberingTextFallback)
                    {
                        paragraph.Type = ElementType.ListItem;
                        var text = childGroup.Contents.OfType<TextToken>().FirstOrDefault();
                        paragraph.ListTextFallback = text?.Value ?? string.Empty;
                        var font = childGroup.Contents.OfType<Font>().FirstOrDefault();
                        paragraph.ListTextFont = font?.Name ?? string.Empty;
                    }
                    else if (dest?.Type == TokenType.HeaderTag
                    ...
                    else if (dest is BookmarkStart && childGroup.Contents.OfType<TextToken>().FirstOrDefault()?.Value is string id)
                    {
                        paragraph.Add(new Anchor(AnchorType.Bookmark, id));
                    }
                    ...
                    else if (dest is ShapeTag)
                    {
                        ...
                        {
                            paragraph.Type = ElementType.Shape;
                            //paragraph.ShapeElement = shape;
                        }
                    }
    ...
    private void OrganizeLists(Element root)
    {
          ...
          .Where(e => e.Type == ElementType.ListItem))
        {
            ...
            if (listItem.Parent != null && (listItem.Parent.Type == ElementType.TableCell || listItem.Parent.Type == ElementType.TableHeaderCell))
            {
